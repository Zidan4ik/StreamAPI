...
public class StreamIntermediate {
    public static void main(String[] args) {
        ...
        Stream<Integer> map = Stream.of(1, 6, 3, 1, 8, 92, 51, 12, 0, -5).map(value -> value + 10);

        Stream<Integer> distinct = Stream.of(1, 6, 3, 1, 8, 92, 51, 12, 0, -5).distinct();

        Stream<Integer> skip = Stream.of(1, 6, 3, 1, 8, 92, 51, 12, 0, -5).skip(5);

        IntStream mapToInt = Stream.of("1", "6", "3", "1", "8", "92", "51", "12", "0", "-5").mapToInt(value -> Integer.parseInt(value));
...
//        List<Integer> collect = filter2.filter(x -> x >= 5).collect(Collectors.toList());
//        filter2.collect(Collectors.toList());

        // no use more than once instances type of Stream, as instance
//        Stream<Integer> stream = filter2.filter(x -> x >= 5);
//        List<Integer> collectFilter2 = stream.collect(Collectors.toList());
